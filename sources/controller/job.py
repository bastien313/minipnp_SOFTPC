...
class FeederBeforeCmdTask(SimpleTask):
    """
    ...
    def _nextRequest(self):
        self._feeder.prepareBeforePick()
        return TaskStatus(status=TaskStatusEnum.END)
...
class WaitPresureTask(SimpleTask):
    ...
    def __init__(self, pnpDriver, delayInS, presureTarget, feeder, name=''):
        """
        :param delayInS: Delay of wait time in seconds.
        """
        super().__init__(name)
...
class FeederSuccesNotify(SimpleTask):
    """
    ...
    def __init__(self, feeder, name=''):
        ...
        super().__init__(name)
        self._feeder = feeder
        self._taskConfigure([self._feederNotify])

    def _feederNotify(self):
...
class PickAndPlaceJob(Job):
    ...
    def __init__(self, pnpDriver, feeder, placePos, model, zLift, correctorPos, name='', ref=''):
        ...
        if model.correctorMode == 'Mechanical':
            correctorTask = MechanicsCorrectorJob(pnpDriver, correctorPos, self._model, zLift)
        elif model.correctorMode == 'Optical':
        ...
        self._taskList = [
            ...
            WaitPresureTask(self._driver, model.pickupDelay / 1000.0, model.pressureTarget, feeder,
...
class MechanicsCorrectorJob(Job):
    """
    """
